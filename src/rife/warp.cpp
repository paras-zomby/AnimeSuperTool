...
#include "rife_warp.comp.hex.h"
#include "rife_warp_pack4.comp.hex.h"
#include "rife_warp_pack8.comp.hex.h"
...
int RifeWarp::create_pipeline(const Option& opt)
{
    if (!vkdev)
        return 0;
    ...
    // pack1
    {
        ...
        static ncnn::Mutex lock;
        {
            ...
            if (spirv.empty())
            {
                compile_spirv_module(rife_warp_comp_data, sizeof(rife_warp_comp_data), opt, spirv);
            }
        }
    ...
    // pack4
    {
        ...
        static ncnn::Mutex lock;
        {
            ...
            if (spirv.empty())
            {
                compile_spirv_module(rife_warp_pack4_comp_data, sizeof(rife_warp_pack4_comp_data), opt, spirv);
            }
        }
    ...
    if (opt.use_shader_pack8)
    {
        ...
        static ncnn::Mutex lock;
        {
            ...
            if (spirv.empty())
            {
                compile_spirv_module(rife_warp_pack8_comp_data, sizeof(rife_warp_pack8_comp_data), opt, spirv);
            }
        }
...
int RifeWarp::destroy_pipeline(const Option& opt)
{
...
int RifeWarp::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
...
int RifeWarp::forward(const std::vector<VkMat>& bottom_blobs, std::vector<VkMat>& top_blobs, VkCompute& cmd, const Option& opt) const
{
