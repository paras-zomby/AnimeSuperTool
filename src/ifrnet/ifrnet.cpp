...
DEFINE_LAYER_CREATOR(IFRNetWarp)

IFRNet::IFRNet(int gpuid, std::string model_path_dir, int scale, bool _tta_mode, int _num_threads, int type, bool GoProversion)
{
    ...
    if (scale > 1)
    {
        ...
        model_path_prefix << model_path_dir << "/IFRNet_" << type_str
                            << (GoProversion ? "GoPro" : "Vimeo90K");
        load(model_path_prefix.str());
    }
}
...
int IFRNet::load(const std::string& modeldir)
{
    ...
    ifrnet.register_custom_layer("ifrnet.Warp", IFRNetWarp_layer_creator);
...
{
    outimage.create(in0image.w, in0image.h, in0image.elemsize, in0image.elempack);
