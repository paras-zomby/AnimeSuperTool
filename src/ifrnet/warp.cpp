...
#include "ifrnet_warp.comp.hex.h"
#include "ifrnet_warp_pack4.comp.hex.h"
#include "ifrnet_warp_pack8.comp.hex.h"
...
int IFRNetWarp::create_pipeline(const Option& opt)
{
    if (!vkdev)
        return 0;
    ...
    // pack1
    {
        ...
        static ncnn::Mutex lock;
        {
            ...
            if (spirv.empty())
            {
                compile_spirv_module(ifrnet_warp_comp_data, sizeof(ifrnet_warp_comp_data), opt, spirv);
            }
        }
    ...
    // pack4
    {
        ...
        static ncnn::Mutex lock;
        {
            ...
            if (spirv.empty())
            {
                compile_spirv_module(ifrnet_warp_pack4_comp_data, sizeof(ifrnet_warp_pack4_comp_data), opt, spirv);
            }
        }
    ...
    if (opt.use_shader_pack8)
    {
        ...
        static ncnn::Mutex lock;
        {
            ...
            if (spirv.empty())
            {
                compile_spirv_module(ifrnet_warp_pack8_comp_data, sizeof(ifrnet_warp_pack8_comp_data), opt, spirv);
            }
        }
...
int IFRNetWarp::destroy_pipeline(const Option& opt)
{
...
int IFRNetWarp::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    const Mat& image_blob = bottom_blobs[1];
...
int IFRNetWarp::forward(const std::vector<VkMat>& bottom_blobs, std::vector<VkMat>& top_blobs, VkCompute& cmd, const Option& opt) const
{
    const VkMat& image_blob = bottom_blobs[1];
