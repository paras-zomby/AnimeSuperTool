...
RealCUGAN::RealCUGAN(int gpuid, std::string model_path_dir, int _tilenum, int zoom_scale, 
                        int num_threads, bool denoise, int _syncgap, bool _tta_mode)
{
    vkdev = gpuid == -1 ? 0 : ncnn::get_gpu_device(gpuid);

    net.opt.num_threads = num_threads;

    if (zoom_scale < 1 || zoom_scale > 4)
    {
        fprintf(stderr, "unsupported zoom scale %d\n", zoom_scale);
        return;
    }
    if (_tilenum != 1 && _tilenum != 2 && _tilenum != 4)
    {
        fprintf(stderr, "unsupported tile num %d\n", _tilenum);
        return;
    }
    ...
    if (denoise || zoom_scale > 1)
    {
        if (zoom_scale == 1)
            zoom_scale = 2;
        std::stringstream model_path_prefix;
        model_path_prefix << model_path_dir << "/up" << zoom_scale <<
                            "x-" << (denoise ? "denoise3x." : "no-denoise.");
        load(model_path_prefix.str() + "param", model_path_prefix.str() + "bin");
    }
}
...
int RealCUGAN::process(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    outimage.create(inimage.w * scale, inimage.h * scale, inimage.elemsize, inimage.elempack);
    ...
    const int xtiles = (tilenum > 1) ? 2 : 1;
    const int ytiles = (tilenum > 2) ? 2 : 1;
    const int TILE_SIZE_X = w / xtiles;
    const int TILE_SIZE_Y = h / ytiles;
